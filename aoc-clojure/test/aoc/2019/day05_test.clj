...
(deftest with-inputs-test
  (is (= (:out (compute (init-state "3,0,4,0,99" 77))) 77))
  (is (= (:out (compute (init-state "3,9,8,9,10,9,4,9,99,-1,8" 8))) 1))
  (is (= (:out (compute (init-state "3,9,8,9,10,9,4,9,99,-1,8" 42))) 0))
  (is (= (:out (compute (init-state "3,9,7,9,10,9,4,9,99,-1,8" 7))) 1))
  (is (= (:out (compute (init-state "3,9,7,9,10,9,4,9,99,-1,8" 8))) 0))
  (is (= (:out (compute (init-state "3,3,1108,-1,8,3,4,3,99" 8))) 1))
  (is (= (:out (compute (init-state "3,3,1107,-1,8,3,4,3,99" 7))) 1)))

(deftest long-input-test
  (let [i "3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,\n1106,0,36,98,0,0,1002,21,125,20,4,20,1105,1,46,104,\n999,1105,1,46,1101,1000,1,20,4,20,1105,1,46,98,99"]
    (is (= 999 (:out (compute (init-state i 0)))))
    (is (= 1000 (:out (compute (init-state i 8)))))
    (is (= 1001 (:out (compute (init-state i 16)))))))
